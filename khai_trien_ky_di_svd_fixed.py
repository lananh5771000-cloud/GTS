...
def condition_number_pdf(
    ...
    *,
    ...
    """
    Tính số điều kiện đúng theo thuật toán PDF.

    B1: Nếu rank(A) < min(m,n) hoặc A = O thì cond(A) = ∞.
    B2: Nếu m ≥ n đặt T = AᵀA, ngược lại đặt T = AAᵀ.
    B3: Tìm λmax của T bằng phương pháp lũy thừa.
    B4: Tìm λ′max của T⁻¹ bằng phương pháp lũy thừa.
    B5: λmin = 1/λ′max, cond(A) = sqrt(λmax/λmin).
    """
    A = np.asarray(matrix, dtype=float)
    if A.ndim != 2:
        raise ValueError("A phải là ma trận hai chiều.")
    if epsilon <= 0.0 or max_iter <= 0:
        raise ValueError("epsilon và max_iter phải dương.")
    if A.size == 0 or min(A.shape) == 0:
        return math.inf
    if not np.all(np.isfinite(A)):
        raise ValueError("A phải có các phần tử hữu hạn.")

    if np.allclose(A, 0.0) or np.linalg.matrix_rank(A) < min(A.shape):
        return math.inf

    T = A.T @ A if A.shape[0] >= A.shape[1] else A @ A.T
    ...
    lambda_prime_max = _dominant_eigenvalue_by_power(T_inverse, epsilon, max_iter)
    if not math.isfinite(lambda_prime_max) or lambda_prime_max <= 0.0:
        return math.inf
