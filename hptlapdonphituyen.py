...
ALLOWED_FUNCTIONS = {
    ...
    "log": sp.log, "sqrt": sp.sqrt, "abs": sp.Abs, "Abs": sp.Abs,
    ...
    for i in range(n):
        while True:
            try:
                ...
                equations.append(parse_equation(input(prompt), symbols)); break
            except InputError as exc:
                print(f"[!] {exc}")
    return equations
...
def interval_evaluate(expr: sp.Expr, symbols, bounds):
    ...
    def walk(e):
        ...
        if e.is_Symbol:
            if e not in env: raise ValueError(f"bien la {e}")
            return env[e]
        ...
        if e.func is sp.Mul:
            ...
            for a in args: out *= a
            return out
        if e.func is sp.Pow: return _interval_pow(args[0], e.args[1])
        if e.func is sp.sin: return mp.iv.sin(args[0])
        if e.func is sp.cos: return mp.iv.cos(args[0])
        if e.func is sp.tan:
            lo, hi = _ends(args[0])
            k0 = math.ceil((lo - math.pi / 2) / math.pi)
            k1 = math.floor((hi - math.pi / 2) / math.pi)
            if k0 <= k1: raise ValueError("tan gap diem cuc")
            return mp.iv.tan(args[0])
        if e.func is sp.exp: return mp.iv.exp(args[0])
        if e.func is sp.log:
            lo, _ = _ends(args[0])
            if lo <= 0: raise ValueError("đối số log có thể không dương")
            return mp.iv.log(args[0])
        if e.func is sp.Abs: return abs(args[0])
...
def adaptive_interval_evaluate_with_meta(expr: sp.Expr, symbols, bounds,
    ...
    while queue and boxes < max_boxes:
        ...
        if narrow or (valid and depth >= max_depth):
            accepted.append((lo, hi)); continue
        if depth >= max_depth:
        ...
        left[j] = (box[j][0], mid); right[j] = (mid, box[j][1])
    ...
    if not accepted:
        return result(math.nan, math.nan, False,
                      terminal_reason or "không đánh giá được")
    return result(min(v[0] for v in accepted), max(v[1] for v in accepted),
                  True, "")
...
def check_mapping_subset_with_meta(phi, symbols, bounds):
    ...
    ranges = refined
    ok = domain_ok and all(valid and a-TOL <= lo and hi <= b+TOL
                           for (lo, hi, valid, _), (a, b) in zip(ranges, bounds))
    meta = {"method": "adaptive_subdivision" if adaptive_used else "direct_interval",
            "boxes_used": boxes_used, "max_depth_used": max_depth_used}
...
def bound_jacobian(phi, symbols, bounds):
    ...
    for expr in phi:
        ...
        for x in symbols:
            ...
            drow.append(derivative); irow.append((lo, hi, good, reason))
            mrow.append(max(abs(lo), abs(hi)) if good else math.inf)
            valid &= good
        derivs.append(drow); intervals.append(irow); matrix.append(mrow)
    q_inf = max((sum(row) for row in matrix), default=0.0)
    q_one = max((sum(matrix[i][j] for i in range(len(phi)))
                 for j in range(len(symbols))), default=0.0)
...
class Candidate:
    ...
    def banach_proven(self):
        return (self.domain_proven and self.mapping_proven
...
def generate_isolation_candidates(F, symbols):
    ...
    for r, equation in enumerate(F):
        for x in symbols:
            ...
            for root in roots:
                ...
                forms = []
                for transform in (sp.simplify, sp.cancel, sp.factor,
                                  sp.trigsimp, lambda e: sp.powsimp(e, force=False)):
                    try: forms.append(transform(verification))
                ...
                if not verified:
                    for value in forms:
                        try:
                            ...
                            continue
                if (verified and x not in root.free_symbols
    ...
    def visit(j, used, selected):
        ...
        for r, expr in choices[symbols[j]]:
            if r not in used: visit(j + 1, used | {r}, selected + [(r, expr)])
    visit(0, set(), [])
    return candidates
...
def generate_relaxation_candidates(F, symbols, bounds, x0):
    ...
    for i, (f, x) in enumerate(zip(F, symbols)):
        try: d = float(sp.N(sp.diff(f, x).subs(dict(zip(symbols, point)))))
        except (TypeError, ValueError, ZeroDivisionError): return []
        if not math.isfinite(d) or abs(d) < TOL: return []
        bases.append(1.0 / d)
    result = []
    for factor in (0.25, 0.5, 1.0, 1.5):
        phi = [sp.simplify(x - factor * lam * f) for x, lam, f in zip(symbols, bases, F)]
        result.append(Candidate(phi, "Lặp đơn thư giãn",
...
def textbook_interval(expr, symbols, bounds):
    ...
    def walk(node):
        if node.is_Number:
            value = float(node); return value, value
        if node.is_Symbol:
            if node not in env: raise ValueError
            return env[node]
        ...
        if node.func is sp.Mul:
            out = (1.0, 1.0)
            for child in node.args: out = mul(out, walk(child))
            return out
        if node.func in (sp.sin, sp.cos): return -1.0, 1.0
        if node.func is sp.Pow and node.args[1].is_Integer:
            base = walk(node.args[0]); exponent = int(node.args[1])
            if exponent >= 0:
                values = [base[0]**exponent, base[1]**exponent]
                if exponent % 2 == 0 and base[0] <= 0 <= base[1]: values.append(0.0)
                return min(values), max(values)
...
def format_fraction(numerator, denominator, symbol_map=None) -> str:
    ...
    if denominator.is_Integer and abs(int(denominator)) != 1 \
            and numerator.func is not sp.Add and not numerator.is_Number:
        ...
        if coefficient.is_Rational:
            ...
            coefficient_text = format_math_expr(abs(combined), symbol_map,
    ...
    denominator_text = format_math_expr(denominator, symbol_map,
                                        allow_fraction=False)
    numerator_text = (f"({numerator_text})" if numerator.func is sp.Add
                      else numerator_text)
...
def format_product(expr, symbol_map=None) -> str:
    ...
    if coefficient != 1:
        ...
        pieces.append(f"({text})" if coefficient.is_Rational and coefficient.q != 1
                      else text)
...
def format_math_expr(expr, symbol_map=None, allow_fraction=True):
    ...
    if expr.func is sp.Add:
        ...
        for term in expr.as_ordered_terms():
            ...
            text = format_math_expr(-term if negative else term, symbol_map,
                                    allow_fraction=False)
            parts.append(("−" if negative else "+", text))
        out = ("−" if parts[0][0] == "−" else "") + parts[0][1]
        for sign, text in parts[1:]: out += sign + text
        return out
    ...
    if expr.func in (sp.sin, sp.cos, sp.tan, sp.exp, sp.log, sp.Abs):
        names = {sp.sin:"sin", sp.cos:"cos", sp.tan:"tan", sp.exp:"exp",
                 sp.log:"ln", sp.Abs:"|"}
...
def format_interval(interval, precision=7):
    return (f"[{format_exact_or_decimal(interval[0], precision)}, "
            f"{format_exact_or_decimal(interval[1], precision)}]")


def format_equation_system(expressions, left_prefix="x"):
    lines = ["⎧ " + f"{left_prefix}{str(i+1).translate(SUBSCRIPT)} = {format_math_expr(expr)}"
...
def explain_interval_bound(expr, symbols, bounds, precision=7,
                           certified_range=None, adaptive_used=False):
    """Phân tích cây SymPy và sinh các bước chặn khoảng có kiểm chứng."""
    seen, steps = set(), []
    def visit(node):
        key = sp.srepr(node)
        lo, hi, valid, reason = interval_evaluate(node, symbols, bounds)
        if not valid: return lo, hi, False, reason
        if node.is_Symbol:
            if key not in seen:
                steps.append(f"{format_math_expr(node)} ∈ {format_interval((lo,hi), precision)}.")
            ...
            for child in children:
                ...
                if not good: return clo, chi, False, why
            ...
            if key not in seen:
                ...
                if node.func in (sp.sin, sp.cos, sp.tan, sp.exp, sp.log, sp.Abs):
                    ...
                    steps.append(f"{format_math_expr(node.args[0])} ∈ {source} ⇒ {text} ∈ {format_interval((lo,hi), precision)}.")
                elif node.func is sp.Pow and node.args[1] == sp.Rational(1,2):
                    steps.append(f"{format_math_expr(node.args[0])} ∈ {format_interval(child_ranges[0], precision)} ⇒ {text} ∈ {format_interval((lo,hi), precision)}.")
                else:
                    sources = ", ".join(format_interval(r, precision) for r in child_ranges)
                    steps.append(f"Từ các khoảng {sources}, suy ra {text} ∈ {format_interval((lo,hi), precision)}.")
        seen.add(key)
        return lo, hi, True, ""
    ...
    if certified_range is not None and adaptive_used:
        ...
        steps.append(f"{format_math_expr(expr)}(D) ⊂ {format_interval(certified_range, precision)}.")
...
def explain_absolute_bound(expr, symbols, bounds, precision=7):
    ...
    lo, hi, valid, reason = interval_evaluate(expr, symbols, bounds)
    if not valid: return [], math.inf, math.inf, False, reason
    rigorous = max(abs(lo), abs(hi))
    details = []
    def simple(node):
        if node.is_Number: return abs(float(node))
        if node.is_Symbol:
            i = list(symbols).index(node); value = max(abs(bounds[i][0]), abs(bounds[i][1]))
            details.append(f"|{format_math_expr(node)}| ≤ {format_exact_or_decimal(value, precision)}")
            return value
        if node.func is sp.Mul:
        ...
        if node.func is sp.Pow and node.args[1].is_Integer:
            ...
            if exponent >= 0: return simple(node.args[0]) ** exponent
        if node.func is sp.Abs: return simple(node.args[0])
    ...
    steps.append(f"Do đó |{format_math_expr(expr)}| ≤ {format_exact_or_decimal(textbook, precision)}.")
...
def prepare_textbook_bounds(candidate, symbols, bounds, precision=7):
    ...
    for i, row in enumerate(derivs):
        ...
        for j, derivative in enumerate(row):
            ...
            simple_good = (good and math.isfinite(simple_bound)
        ...
        simple_M.append(mrow); explanations.append(erow)
    qi = max((sum(row) for row in simple_M), default=0.0)
    q1 = max((sum(simple_M[i][j] for i in range(len(simple_M)))
              for j in range(len(simple_M))), default=0.0)
    if qi < 1:  # chuẩn hàng thường thuận tiện nhất để trình bày
        ...
        rq1 = max((sum(rigorous_M[i][j] for i in range(len(rigorous_M)))
                   for j in range(len(rigorous_M))), default=0.0)
        chosen_M, q, norm = rigorous_M, min(rqi, rq1), ("vo cung" if rqi <= rq1 else "1")
        explanations = []
        for i, row in enumerate(derivs):
            erow = []
            for j, derivative in enumerate(row):
                lo, hi, good, _ = intervals[i][j]
                erow.append([f"{format_math_expr(derivative)}(D) ⊂ {format_interval((lo,hi), precision)}, "
                             f"nên |{format_math_expr(derivative)}| ≤ {format_exact_or_decimal(rigorous_M[i][j], precision)}."] if good else [])
...
def bound_system_lipschitz(F, symbols, bounds):
    ...
    for equation in F:
        ...
        lo, hi, good, _ = adaptive_interval_evaluate(
            equation, symbols, bounds, max_depth=7, max_boxes=2048,
            target_width=None)
        if not good: return math.inf, math.inf
        scale = max(scale, abs(lo), abs(hi))
        for symbol in symbols:
            derivative = sp.diff(equation, symbol)
            dlo, dhi, valid, _ = adaptive_interval_evaluate(
                derivative, symbols, bounds, max_depth=7,
                max_boxes=2048, target_width=None)
            if not valid: return math.inf, math.inf
...
def evaluate_real_vector(exprs, symbols, point):
    ...
    for i, expr in enumerate(exprs):
        try:
            ...
            raise ArithmeticError(f"φ{str(i+1).translate(SUBSCRIPT)} không tính được: {exc}") from exc
...
def compute_error_bounds(q, norm, x_new, x_old, phi_at_new):
    if not math.isfinite(q) or q >= 1: return None, None, None
...
def fixed_point_iteration(phi, symbols, x0, bounds, q, norm, F=None,
                          epsilon=None, relative=False, exact_steps=None,
                          max_iter=1000, banach_proven=False,
                          equation_residual_tolerance=None,
                          F_lipschitz=None, F_scale=None):
    ...
    def residual_consistent(residual, error_bound):
        ...
        if residual is None or not math.isfinite(residual): return False
        if error_bound is None:
            return residual <= round_tolerance
        if F_lipschitz is None or not math.isfinite(F_lipschitz): return False
    ...
    for k in range(1, limit + 1):
        ...
        if exact_steps is None and epsilon is not None:
            ...
            relative_threshold = math.inf if q == 0 else (1-q)/q * epsilon
            error_ok = (rel is not None and rel <= relative_threshold) if relative else (
                error is not None and error <= epsilon)
...
def validate_printed_proof(candidate, bounds):
    ...
    if len(candidate.proof_bounds) != len(bounds):
        return False
    if any(abs(a-c) > TOL or abs(b-d) > TOL
    ...
    q_one = max((sum(M[i][j] for i in range(len(M)))
                 for j in range(len(M[0]) if M else 0)), default=0.0)
    expected = q_inf if candidate.selected_norm == "vo cung" else q_one
    return (candidate.selected_norm in ("vo cung", "1")
            and math.isfinite(candidate.q) and candidate.q < 1
            and abs(candidate.q-expected) <= 100*TOL*max(1, abs(expected)))


def print_exam_proof(candidate, symbols, bounds, F=None, direct=False,
                     original_bounds=None, precision=7, stop_kind="absolute"):
    if not validate_printed_proof(candidate, bounds):
        print("Dữ liệu chứng minh nội bộ không nhất quán, không thể xuất lời giải bảo đảm.")
        return False
    ...
    if original_bounds is not None and original_bounds != bounds:
        print(f"Trên miền ban đầu D = {' × '.join(format_interval(v, precision) for v in original_bounds)},")
        ...
        violated = []
        for i, ((lo, hi, valid, _), target) in enumerate(zip(old_ranges, original_bounds)):
            if valid:
                old_steps, _, _, _ = explain_interval_bound(candidate.phi[i],
                    symbols, original_bounds, precision, (lo, hi),
                    old_meta["method"] == "adaptive_subdivision")
                for step in old_steps: print("  " + step)
            contained = valid and target[0]-TOL <= lo and hi <= target[1]+TOL
            relation = "⊂" if contained else "⊄"
            connector = "và" if contained else "nhưng"
            print(f"  φ{i+1}(D) ⊂ {format_interval((lo, hi), precision)}, "
                  f"{connector} {format_interval((lo, hi), precision)} {relation} {format_interval(target, precision)}.")
            if not contained: violated.append(i+1)
        if not old_ok:
            print("Các thành phần vi phạm: " + ", ".join(f"φ{i}" for i in violated) + ".")
            print("Vì vậy Φ(D) không nằm trong D.")
        print(f"Chọn miền con D′ = {' × '.join(format_interval(v, precision) for v in bounds)} ⊂ D.")
    ...
    if candidate.mapping_method == "adaptive_subdivision":
        ...
        print((f"Dùng chia miền thích nghi ({boxes} hộp), hợp các khoảng thu được:" 
               if boxes else "Dùng chia miền thích nghi, hợp các khoảng thu được:"))
    for i, ((lo, hi, valid, reason), (a, b)) in enumerate(zip(ranges, bounds)):
        if valid:
            steps, _, _, _ = explain_interval_bound(candidate.phi[i], symbols,
                bounds, precision, (lo, hi),
                candidate.mapping_method == "adaptive_subdivision")
            for step in steps: print("  " + step)
            print(f"  φ{i+1} = {format_math_expr(candidate.phi[i])} ∈ {format_interval((lo, hi), precision)}")
            contained = a-TOL <= lo and hi <= b+TOL
            relation = "⊂" if contained else "⊄"
            print(f"  và {format_interval((lo, hi), precision)} {relation} {format_interval((a,b), precision)}.")
        else:
    ...
    for i, row in enumerate(derivs):
        for j, derivative in enumerate(row):
            print(f"  ∂φ{i+1}/∂{_symbol_text(symbols[j])} = {format_math_expr(derivative)}.")
            explanations = getattr(candidate, "bound_explanations", None)
            steps = (explanations[i][j] if explanations is not None
    ...
    if candidate.selected_norm == "vo cung":
        ...
        print("  q = ||JΦ||∞ ≤ max{" + ", ".join(format_number(v, precision) for v in sums)
        ...
        print("  q = ||JΦ||₁ ≤ max{" + ", ".join(format_number(v, precision) for v in sums)
              + f"}} = {format_exact_with_decimal(candidate.q, precision)} < 1.")
    print(f"Vậy Φ là ánh xạ co với hệ số co q = {format_exact_with_decimal(candidate.q, precision)}.")

    print("\nPHẦN 5. KẾT LUẬN HỘI TỤ")
    if candidate.banach_proven:
        print("Theo định lý ánh xạ co Banach, hệ có duy nhất một nghiệm trong miền đã chọn")
...
def print_iteration_formula(candidate, symbols, x0, precision=7,
                            bounds=None, midpoint=False):
    print("\nPHẦN 6. CHỌN GIÁ TRỊ BAN ĐẦU")
    if midpoint and bounds is not None:
        terms = [f"({format_exact_or_decimal(a, precision)}+{format_exact_or_decimal(b, precision)})/2"
                 for a, b in bounds]
...
def exam_main():
    ...
    if mode == 2:
        ...
        for i in range(n):
            while True:
                try:
                    phi_direct.append(parse_expression(input(f"φ_{i+1} = "), symbols)); break
                except InputError as exc:
                    print(f"Lỗi: {exc}")
    ...
    if initial_choice == 1:
        print("Nhập t₀:"); x0 = read_vector(n, "x")
    ...
    epsilon = None; relative = False; steps = None; max_iter = 1000
    if stop in (1, 2):
        epsilon = read_number("δ = ", positive=True); relative = stop == 2
        max_iter = read_int("max_iter = ", 1)
    elif stop == 3:
        steps = read_int("k = ", 0)
    elif stop == 4:
        d = read_int("d = ", 0); epsilon = 0.5 * 10 ** (-d)
        max_iter = read_int("max_iter = ", 1)
    else:
        raise InputError("Lựa chọn điều kiện dừng không hợp lệ.")

    stop_kind = "relative" if relative else ("absolute" if epsilon is not None else "fixed")
    if not print_exam_proof(chosen, symbols, active_bounds, F, mode == 2,
                            bounds, precision, stop_kind):
        return
    ...
    result = fixed_point_iteration(
        chosen.phi, symbols, x0, active_bounds, chosen.q, chosen.selected_norm,
        F, epsilon, relative, steps, max_iter, banach_proven=True,
        equation_residual_tolerance=epsilon if epsilon is not None else None,
        F_lipschitz=F_lipschitz, F_scale=F_scale)
    ...
    if epsilon is not None and last is not None:
        if relative:
            ...
            print(f"Eₖ = q/(1-q)||tₖ-tₖ₋₁|| = {format_number(last[2], precision)} "
                  f"≤ δ = {format_number(epsilon, precision)}.")
    print("\nPHẦN 10. KẾT QUẢ")
    status = result["status"]
    if status == "certified":
        print(f"Nghiệm gần đúng:\nt* ≈ t_{result['steps']} = {format_vector(result['x'], precision)}.")
        print(f"Sai số {'tương đối' if relative else 'tuyệt đối'} không vượt quá δ = {format_number(epsilon, precision)}.")
        ...
        print(f"Vector cuối cùng trước khi dừng: {format_vector(result['x'], precision)}.")
