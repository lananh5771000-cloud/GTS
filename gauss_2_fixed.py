    ...
    while pivot_row < m:
        ...
        for r in range(m):
            if r == pivot_row or aug[r][j0] == 0:
                continue
            # Theo PDF: đặt c = s[i0,j0], λ = s[k,j0] / c,
            # rồi thực hiện R_k <- R_k - λ R_i0. Không được dùng trực tiếp
            # s[k,j0] làm hệ số, vì khi pivot không bằng ±1 thì cột pivot
            # sẽ không bị khử về 0.
            factor = aug[r][j0] / pivot_value
