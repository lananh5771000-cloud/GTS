...
- In bài giải theo đúng các ký hiệu trong tài liệu:
      ...
      q = ||I - A.T||_1, λ = max|a_ii|/min|a_ii| (chéo trội cột)
      X^(k) = α.X^(k-1) + β
...
def ask_choice(prompt: str, choices: Sequence[str], default: Optional[str] = None) -> int:
    allowed = set(choices)
    while True:
        suffix = f" [mặc định {default}]" if default is not None else ""
        raw = input(prompt + suffix + ": ").strip()
...
def matrix_lines(M: np.ndarray, decimals: int = 6) -> List[str]:
    ...
    strings = [
        [format_display_number(value, decimals, scale) for value in row]
        for row in M
    ]
    widths = [max(len(strings[i][j]) for i in range(len(strings))) for j in range(M.shape[1])]
...
def row_dominance_details(A: np.ndarray) -> Tuple[bool, List[Tuple[float, float, bool]]]:
...
def column_dominance_details(A: np.ndarray) -> Tuple[bool, List[Tuple[float, float, bool]]]:
    details: List[Tuple[float, float, bool]] = []
...
def _perfect_matching(ratios: np.ndarray, threshold: float) -> Optional[Tuple[int, ...]]:
    """Tìm hoán vị hàng sao cho mọi ratio <= threshold."""
    n = ratios.shape[0]
    candidates = [
        [r for r in range(n) if math.isfinite(ratios[i, r]) and ratios[i, r] <= threshold]
        for i in range(n)
    ]
...
def _best_dominant_permutation(A: np.ndarray, kind: str) -> Optional[Tuple[int, ...]]:
    ...
    thresholds = sorted({float(x) for x in ratios.ravel() if math.isfinite(x) and x < 1.0})
...
def prepare_jacobi_system(A: np.ndarray, B: np.ndarray) -> Dict[str, object]:
    """
...
def _relative_error_bounds(X: np.ndarray, absolute_bounds: np.ndarray, p: str) -> np.ndarray:
...
def jacobi_solve(
    ...
    *,
    ...
    if stop_mode in ("posteriori_absolute", "posteriori_relative", "apriori") and epsilon <= 0:
        raise ValueError("ε phải dương")
    if fixed_steps < 0:
        raise ValueError("số bước không được âm")

    prepared = prepare_jacobi_system(A, B)
    ...
    while k < max_iter and status is None:
        ...
        history.append(
            {
                ...
                "apriori_bounds": None if apriori_bounds is None else apriori_bounds.copy(),
        ...
        if exact_fixed_point:
            ...
            status = "converged"
        elif stop_mode == "posteriori_absolute" and float(np.max(error_bounds)) <= epsilon:
            status = "converged"
...
def numeric_formula_lines(alpha: np.ndarray, beta: np.ndarray, precision: int) -> List[str]:
    n, m = beta.shape
    lines: List[str] = []
    if m == 1:
        for i in range(n):
            ...
            for j in range(n):
                ...
                if _is_effectively_zero(coefficient, max(float(np.max(np.abs(alpha))), np.finfo(float).tiny)):
                    continue
...
def _component_substitution_lines(
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            x_text = format_display_number(X_previous[j, 0], precision, max(float(np.max(np.abs(X_previous))), np.finfo(float).tiny))
            term = f"{coeff_text}·({x_text})"
            expression += (" + " if coefficient > 0 else " - ") + term
        result_text = format_display_number(
            X_current[i, 0], precision, max(float(np.max(np.abs(X_current))), np.finfo(float).tiny)
        )
...
def _print_stop_condition(result: Dict[str, object]) -> None:
    ...
    if mode == "posteriori_absolute":
        ...
        print(
            f"Trước hết E_k,j = λq/(1-q)·‖X_j^(k)-X_j^(k-1)‖_{p_symbol}."
        )
        print(
            "Chặn sai số tương đối: r_k,j ≤ E_k,j / "
            f"(‖X_j^(k)‖_{p_symbol} - E_k,j)."
        )
...
def print_solution(
    ...
    *,
    ...
    if status in ("not_dominant", "zero_diagonal", "no_contraction"):
        ...
        print("Kết luận: không đủ điều kiện bảo đảm hội tụ theo phương pháp trong tài liệu.")
    ...
    print(f"⇒ A{'\'' if permutation != tuple(range(n)) else ''} là ma trận {_dominance_name(kind)}.")
    ...
    if kind == "row":
        ...
        B1 = np.asarray(result["certificate_matrix"], dtype=float)
        column_sums = np.sum(np.abs(B1), axis=0)
        print("Vì A chéo trội cột nên chọn p = 1.")
        print_matrix("B_1 = I - A.T", B1, decimals)
        print("q = ‖B_1‖_1 = max_j Σ_i|(B_1)_ij|.")
        print("Các tổng cột của |B_1|: " + ", ".join(_sig(x) for x in column_sums) + ".")
    ...
    for index, row in enumerate(history):
        ...
        for j in range(m):
            pieces = [
                f"‖ΔX_{j + 1}‖_{p_symbol} = {_sig(diff_norms[j])}"
            ]
            stop_mode = str(result["stop_mode"])
            if stop_mode == "posteriori_absolute":
                pieces.append(f"E_{k},{j + 1} = {_sig(error_bounds[j])}")
            elif stop_mode == "posteriori_relative":
                rel_text = _sig(relative_bounds[j]) if math.isfinite(relative_bounds[j]) else "chưa xác định"
                pieces.append(f"E_{k},{j + 1} = {_sig(error_bounds[j])}")
            ...
            pieces.append(f"‖A.X_(:,{j + 1})^({k})-B_(:,{j + 1})‖_{p_symbol} = {_sig(residual_norms[j])}")
    ...
    if status == "converged_exact":
        ...
        if stop_mode == "posteriori_absolute":
            ...
            print(f"max_j E_{last_k},j = {_sig(last_error)} ≤ ε = {_sig(float(result['epsilon']))}.")
        elif stop_mode == "posteriori_relative":
            last_relative = float(np.max(np.asarray(last["relative_bounds"], dtype=float)))
            print(f"max_j r_{last_k},j = {_sig(last_relative)} ≤ ε = {_sig(float(result['epsilon']))}.")
        else:
    ...
    if task_mode == 2:
        ...
        left_error = float(np.max(np.sum(np.abs(left - I), axis=1)))
        right_error = float(np.max(np.sum(np.abs(right - I), axis=1)))
...
def main() -> None:
    ...
    decimals = int(
        ask_number("Số chữ số sau dấu phẩy dùng để trình bày", nonnegative=True, integer=True, default=6)
    )
